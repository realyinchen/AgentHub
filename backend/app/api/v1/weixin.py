...
from app.crud import get_user_by_channel_user_id, create_user, get_or_create_user_channel
...
async def weixin_auth_websocket(websocket: WebSocket):
    ...
    try:
        ...
        if not qrcode:
            await websocket.send_json({"type": "error", "message": "Failed to get QR code"})
            return
        ...
        while time.time() - start_time < QRCODE_TIMEOUT:
            ...
            # Poll QR code status
            try:
                status_result = await weixin.poll_status(qrcode, base_url=current_base_url)
            except Exception as e:
            ...
            if status == "confirmed" or status_result.get("bot_token"):
                ...
                bot_base_url = status_result.get("baseurl") or status_result.get("base_url") or current_base_url
                channel_user_id = status_result.get("ilink_user_id")
                
                if not bot_token or not channel_user_id:
                    await websocket.send_json({"type": "error", "message": "Login incomplete"})
                    return
                
                logger.info(f"[WeChat WebSocket] Login confirmed: {channel_user_id}")
                
                # 3. Create or get user
                user_id, thread_id = await _create_weixin_user(channel_user_id, bot_token, bot_base_url)
                ...
                logger.info(f"[WeChat WebSocket] User created: {user_id}, thread: {thread_id}")
                
                # 5. Start message loop (background task)
                asyncio.create_task(create_listener(
                    bot_token=bot_token,
            ...
            if status == "binded_redirect" or status_result.get("binded_redirect"):
                ...
                await websocket.send_json({"type": "error", "message": "Already connected from another session"})
                return
...
async def _create_weixin_user(channel_user_id: str, bot_token: str, bot_base_url: str) -> tuple[UUID, UUID]:
