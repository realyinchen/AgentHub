...
class WeixinService:
    ...
    async def _api_post(
        ...
        try:
            async with session.post(url, json=body, headers=self._make_headers(token)) as resp:
                text = await resp.text()
    ...
    async def get_qrcode(self, local_token_list: list[str] | None = None) -> dict[str, Any]:
        """Get a login QR code.
        ...
        while True:
            ...
            if status == "confirmed" or result.get("bot_token"):
                return {
                    ...
                    "baseurl": result.get("baseurl") or result.get("base_url") or current_base_url,
            ...
            if status in ("need_verifycode", "verify_code_blocked") or result.get("need_verifycode"):
                ...
                return {"success": False, "error": "need_verifycode", "retry": bool(pending_verify_code)}
