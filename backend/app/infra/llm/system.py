...
Configuration is fail-fast: if DEFAULT_LLM_MODEL or DEFAULT_LLM_API_KEY are
missing, `Settings` raises at application startup (see config.py
`validate_system_default_llm`), so by the time this module is called we are
guaranteed both values exist and are valid.
"""
