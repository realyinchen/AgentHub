...
class WeixinListener:
    ...
    def __init__(
        ...
        self._last_contact: dict[str, str | None] = {"from_id": None, "context_token": None}
    ...
    async def start(self) -> None:
        ...
        self._running = True
        logger.info(f"[WeChat] Listener started: user={self.user_id}, thread={self.thread_id}")
    ...
    async def _handle_message(self, msg: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.weixin.send_message(
                self.bot_token, from_user_id, context_token, response.content, self.bot_base_url
            )
            ...
            await self.weixin.send_message(
                self.bot_token, from_user_id, context_token, "抱歉，处理消息时出现错误。", self.bot_base_url
            )
    ...
    async def _reconnect_loop(self) -> None:
        ...
        while self._running:
            ...
            if remaining <= RECONNECT_CONFIG["force_before"]:
                ...
                if self._last_contact["from_id"] and self._last_contact["context_token"]:
                    await self.weixin.send_message(
                        self.bot_token,
