...
1. **测试**：目前未实现单元/集成测试
2. **向量数据库**：向量存储功能（Qdrant 和 sqlite-vec 后端）尚未进行过测试。`vectorstore_search` 工具和文档入库功能需要进一步验证
...
### 数据库优化
- **连接池**：从5增加到20个连接（支持100+并发用户）
- **索引优化**：添加复合索引、部分索引和覆盖索引
- **查询性能**：对话列表查询速度提升5-10倍

### API 优化
- **限流**：默认每IP 100请求/分钟（防止滥用）
- **缓存**：内存TTL缓存模型、供应商、对话（80%+命中率）
- **敏感数据**：日志中API Key脱敏处理

### 前端优化
- **错误边界**：防止白屏崩溃
- **React性能**：useMemo优化和防抖状态更新

### 向量存储优化
- **Qdrant**：HNSW索引配置（m=16, ef=200）实现20-50ms搜索
- **批量搜索**：支持一次请求多个向量查询

详细优化说明请查看 [memory-bank/progress.md](memory-bank/progress.md)。
